...
Path("./simulations_data/lv/{:s}".format(date)).mkdir(parents=True, exist_ok=True)
...
LATENCY_STD_LIST = [ 1, 10, 20, 30, 40, 50, 60, 70, 80, 90 ]
...
print("    LATENCY_STD_LIST:", LATENCY_STD_LIST)
...
# G-LEAP agents with different values of latency variance
for latency_std in LATENCY_STD_LIST:
    ...
    agent = GleapAgent(
        ...
        NUM_SELECTION,
        B,
        name="G-LEAP, $\sigma = {:.1f}$".format(latency_std),
        gamma=GAMMA,
        beta=BETA,
        history_information=history_information,
        v=V,
        h=H)
    filename = "./simulations_data/lv/{:s}/simulation_gleap_lv_{:.1f}.pkl".format(date, latency_std)
    run_agent(env, agent, filename)


total_end_time = time.time()
print("run_lv_simulations.py finished in {:s}.".format(format_time(total_end_time - total_begin_time)))
print("date:", date)
