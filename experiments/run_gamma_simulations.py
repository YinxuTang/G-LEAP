...
date = args.date
print("Running gamma simulations for start date: {:s}".format(date))

# mkdir
from pathlib import Path
Path("./simulations_data/gamma/{:s}".format(date)).mkdir(parents=True, exist_ok=True)
...
GAMMA_ARRAY = np.array([ 0.0, 0.50, 1.00, 2.00, 4.00 ])
...
# GleapAgent with different values of gamma
for gamma in GAMMA_ARRAY:
    env_copy = copy.deepcopy(env)
    agent = GleapAgent(env_copy, NUM_SELECTION, B, name="G-LEAP, $\gamma = {:.2f}$".format(gamma), gamma=gamma, beta=BETA, history_information=history_information, v=V, h=H)
    filename = "./simulations_data/gamma/{:s}/simulation_gleap_gamma_{:.2f}.pkl".format(date, gamma)
    run_agent(env_copy, agent, filename)

total_end_time = time.time()
print("All gamma simulations finished in {:s}.".format(format_time(total_end_time - total_begin_time)))
print("DATE:", date)
